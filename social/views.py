...
from .notifications import (replaceTaggedUsersToTaggedElemesInCom, follow_com_by_sending_sub_com, follow_post,
follow_com, get_post, send_mail_notification, follow_or_unfollow,
tag_user, get_notification_obj_type_from_id, get_notification_obj,
getNotificationsForContentFollowers)
from users.members import get_profile_info, get_active_profile_partial_lst_start_with
from general.general import safeExtend
...
def sub_com_save_ajax(request):
    ...
    if request.method == 'POST' and request.is_ajax:
        ...
        notifications = safeExtend(notifications, tag_user(sub_com))
        notifications = safeExtend(notifications, getNotificationsForContentFollowers(notification.id))
        follow_com_by_sending_sub_com(sub_com)
        replaceTaggedUsersToTaggedElemesInCom(sub_com)

        return JsonResponse({'sub_com_id':str(sub_com.id), 'date':date, 'time':t, 'status':'success', 'body':str(sub_com.body), 'notifications':notifications})
